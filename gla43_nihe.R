...
library(ggpubr)
library(ggpmisc)
ggplot(gla43, mapping = aes(x=group,y=ratio))+
  geom_point(size=1)+
  ...
  ggsave(file="C://Users//Administrator//Desktop//nihe_gla43_lm2.tiff",dpi=500,width=4.5, height=3, units="in")
  
  nls_1 <- nls()
  pra.ml <- lm(ratio~log(group,2) ,data=gla43)
  summary(pra.ml)